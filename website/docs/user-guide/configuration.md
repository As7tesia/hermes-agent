...
```yaml
web:
  backend: firecrawl    # firecrawl | parallel | tavily | brave | exa
```
...
**Backend selection:** If `web.backend` is not set, the backend is auto-detected from available API keys. If only `EXA_API_KEY` is set, Exa is used. If only `TAVILY_API_KEY` is set, Tavily is used. If only `PARALLEL_API_KEY` is set, Parallel is used. If only `BRAVE_API_KEY` is set, Brave Search is used. Otherwise Firecrawl is the default.
